...
import GET_REVIEWS_PAGE from '../../graphql/query/GetReviewsPage';
import GET_REVIEWS from '../../graphql/query/GetReviews';
import Meta from '../../lib/Meta';

/** fetch data at build time */
export const getStaticProps = async () => {
	const { data: page } = await client.query({
		query: GET_REVIEWS_PAGE,
	});
...
const Reviews = ({ page, reviews }) => {
	return (
		<div>
			{/** title bar start */}
			<Meta title="Reviews | An Authentic African Experience | Aerocruise Safaris" />
			{/** title bar end */}
			...
			{/** reviews grid section end */}
			<section className="mx-auto max-w-6xl transform px-4 pb-8 sm:px-6 lg:mt-24 lg:px-8 lg:pb-16">
							...
							<div key={review.id}>
								<div className="block rounded-lg bg-slate-50 shadow-lg">
									...
									<div className="flex flex-col p-6">
										...
										<h4 className="font-semibold">Visited: {review.destination.name}</h4>
										<hr className="divider my-5 w-full" />
										<div
